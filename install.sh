...
jumpBoxSshPublicKeyPath="$(readlink -f ./scripts/.jbSshPublicKey)"#file .jbSshPublicKey is generated by deploy_purdue script and contains the ssh public key installed on the jumpbox
echo $jumpBoxSshPublicKeyPath
./scripts/deploy_iiotassets.sh -s=$subscription -l=$location -rg=$resourceGroupPrefix -vmSize=$vmSize -sshPublicKeyPath=$jumpBoxSshPublicKeyPath
